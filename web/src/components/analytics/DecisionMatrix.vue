...
/**
 * Renders `GET /analytics/decisions` as a heatmap: tools
 ...
 * `{ tool_name, decision_source }` so the host (`/analytics`) can
...
/** Union of every row's `by_source` keys, first-seen order — never a hardcoded list. */
...
<template>
  ...
  >
    ...
    <!-- The heatmap's axis labels are canvas text; this HTML table is where decision_source goes through RawValue and shows exact_share/accept/reject totals the grid alone doesn't. -->
